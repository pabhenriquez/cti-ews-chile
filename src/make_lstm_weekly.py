...
# -*- coding: utf-8 -*-
"""
LSTM variant of the weekly SDML analysis. Same data conventions, detrending,
surrogate split, and evaluation as sdml_pipeline_weekly.py, with a stacked
LSTM ensemble instead of the multi-head CNN.

Usage:
    python src/make_lstm_weekly.py [--method AAFT|FT|IAAFT|RP]
"""
import os
import matplotlib
matplotlib.use('Agg')
...
def sdml_predict_incremental_pre_fixed(t_num, resid_pre_all, L, models, inc, start_idx, stop_idx):
    means, stds, tt, ipos = [], [], [], []
    for i in range(max(L, start_idx) + 1, stop_idx + 1, inc):
        ...
        probs = [float(m.predict(win, verbose=0)[0, 0]) for m in models]
...
def process_dataset(cfg, surrogate_method=SURROGATE_METHOD):
    ...
    for seed in range(N_MODELS):
        ...
        model = build_lstm_binary(L=L_eff)
        es = tf.keras.callbacks.EarlyStopping(monitor="val_loss", mode="min", patience=10, restore_best_weights=True)
        model.fit(X_tr, y_tr, validation_data=(X_val, y_val), epochs=100, batch_size=64, callbacks=[es], verbose=0)
    ...
    print(f"[{cfg['name']}] SDML-LSTM-{surrogate_method} F1 test: {np.mean(f1_list):.3f} ± {np.std(f1_list):.3f}")
...
with open(os.path.join(OUT_DIR, f"results_lstm_{SURROGATE_METHOD}_weekly.pkl"), "wb") as f:
...
PDF_PATH = os.path.join(FIG_DIR, f"LSTM_{SURROGATE_METHOD}_weekly.pdf")
